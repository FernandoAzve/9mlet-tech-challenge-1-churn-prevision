...
2. **Processo antes da ferramenta:** contexto em `.cursor/context` e regras em `.cursor/rules` devem estar atualizados; prompts genéricos geram dívidas técnicas mais rápido.
...
- Respeitar **`.cursor/libs/allowed-libs.md`** e **`.cursor/rules/code-style.md`**.
...
- Seguir **`.cursor/rules/testing-rules.md`** (pytest, nomenclatura `test_*.py`).
...
Consulte **`.cursor/libs/ai-models.md`** para orientação por fase (arquitetura, implementação, depuração). Os nomes comerciais dos modelos mudam com o tempo; priorize o modelo disponível na sua assinatura com maior contexto e melhor desempenho em código Python.
